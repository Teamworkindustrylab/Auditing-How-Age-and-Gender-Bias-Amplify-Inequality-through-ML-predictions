...
KEEP_COLS = [
    ...
    "YearsCode", "YearsCodePro", "DevType", "ConvertedCompYearly",
]

def _parse_so(path: str) -> pd.DataFrame:
    """
    Parse SO survey CSV with pandas python engine.

    The SO export has inconsistent internal quoting (e.g. degree names like
    "Bachelor's degree (B.A., B.S.)" contain unescaped quotes). The pandas
    python engine with on_bad_lines='skip' handles this reliably and, crucially,
    keeps Age and ConvertedCompYearly on the SAME row -- something the old
    manual csv.reader loop failed to do, causing young rows to get no salary.
    """
    ...
    try:
        ...
        print(f"[SO]   Full parse: {len(df):,} rows, {len(df.columns)} columns")
        present = [c for c in KEEP_COLS if c in df.columns]
        missing = [c for c in KEEP_COLS if c not in df.columns]
        if missing:
            print(f"[SO]   WARNING -- columns not found: {missing}")
        df = df[present].copy()

    except Exception as e:
        print(f"[SO]   Full parse failed ({e}), trying usecols fallback ...")
        df = pd.read_csv(
            path,
            encoding="latin-1",
            on_bad_lines="skip",
    ...
    print(f"[SO]   Columns kept: {list(df.columns)}")
    print(f"[SO]   Rows: {len(df):,}")

    if "Age" in df.columns:
        print(f"[SO]   Age sample:\n{df['Age'].value_counts().head(8).to_string()}")
    if "ConvertedCompYearly" in df.columns:
        n_sal = pd.to_numeric(df["ConvertedCompYearly"], errors="coerce").notna().sum()
        print(f"[SO]   ConvertedCompYearly non-null: {n_sal:,} ({n_sal/len(df)*100:.1f}%)")
...
def plot_so_salary_by_age(df: pd.DataFrame):
    ...
    # Debug: show how many rows have salary per group BEFORE any filter
    print(f"\n[SO]   Rows per group (all, incl. missing salary):")
    print(plot_df.groupby("age_group")["salary"].agg(
        total="count", has_salary=lambda x: x.notna().sum()
    ).to_string())

    plot_df = plot_df[plot_df["salary"].notna() & (plot_df["salary"] > 0)].copy()

    print(f"\n[SO]   Rows per group AFTER salary filter:")
    print(plot_df["age_group"].value_counts().to_string())

    # Percentile trim on the full salary distribution (not per group)
    lo = plot_df["salary"].quantile(0.01)
    hi = plot_df["salary"].quantile(0.99)
    plot_df = plot_df[(plot_df["salary"] >= lo) & (plot_df["salary"] <= hi)].copy()

    print(f"\n[SO]   Rows per group AFTER percentile trim (p1={lo:.0f}, p99={hi:.0f}):")
    print(plot_df["age_group"].value_counts().to_string())

    # Guard: if either group is empty the plot would be blank
    for grp in ["young (< 35)", "experienced (35+)"]:
        n = (plot_df["age_group"] == grp).sum()
        if n == 0:
            print(f"  WARNING: group '{grp}' has 0 rows after filtering — "
                  f"check that Age column values match expected format.")
