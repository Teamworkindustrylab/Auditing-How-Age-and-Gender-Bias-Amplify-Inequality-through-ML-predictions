...
class AdultMitigation:
    ...
    def load(self) -> "AdultMitigation":
        if not os.path.exists(IN_ADULT):
            raise FileNotFoundError(
                f"[ADULT MIT] {IN_ADULT} not found -- run Notebook 2 first."
            )
    ...
    def run(self) -> dict:
        ...
        rows = [{"strategy": s, "auc": a, "dpd": d,
                 "meets_threshold": d <= DPD_THRESHOLD}
                for s, (a, d) in self.tradeoff.items()]
        pd.DataFrame(rows).to_csv(f"{OUT}/adult_mitigation_results.csv",
                                  index=False)
        print(f"\n  Saved -> {OUT}/adult_mitigation_results.csv")
        return self.tradeoff
...
def plot_mitigation_tradeoff(so_tradeoffs: dict, fcc_tradeoff: dict,
    ...
    if adult_tradeoff:
        ax_adult = axes[fcc_idx + 1]
        ax_adult.set_facecolor("#eef9f0")
        for strat, (auc, dpd) in adult_tradeoff.items():
            ax_adult.scatter(dpd, auc, s=220, color=PALETTE["adult"],
                             marker=markers[strat], zorder=5,
                             edgecolors="white", lw=1.2, label=labels[strat])
            ax_adult.annotate(labels[strat], (dpd, auc),
                              textcoords="offset points", xytext=(6, 4),
                              fontsize=8)
        ax_adult.axvline(DPD_THRESHOLD, color=PALETTE["bad"],
                         linestyle="--", lw=1.5)
        ax_adult.set_xlabel("|DPD|  ->  lower is fairer")
        ax_adult.set_ylabel("ROC-AUC")
        ax_adult.set_title("UCI Adult -- Gender", fontsize=9, fontweight="bold")
        ax_adult.legend(fontsize=7)
...
def plot_eu_compliance_table(so_tradeoffs: dict, fcc_tradeoff: dict,
    ...
    if adult_tradeoff:
        adult_post_ok = (
            adult_tradeoff["reweigh"][1] <= DPD_THRESHOLD or
            adult_tradeoff["threshold"][1] <= DPD_THRESHOLD
        )
    ...
    if adult_tradeoff:
        adult_extra = [
            ...
            "YES -- SHAP in NB4",
            "NO -- not implemented",
            "PASS" if adult_tradeoff["baseline"][1] <= DPD_THRESHOLD
                    else "NO -- exceeds threshold",
            "PARTIAL" if adult_post_ok else "NO",
        ]
        for row, val in zip(rows, adult_extra):
            row.append(val)
        col_labels.append("UCI Adult (gender bias)")

    fig_w = 14 if not adult_tradeoff else 17
    fig, ax = plt.subplots(figsize=(fig_w, 5), facecolor=PALETTE["bg"])
    ax.axis("off")
...
if __name__ == "__main__":
    ...
    adult_tradeoff = None
    if os.path.exists(IN_ADULT) and os.path.exists(
        os.path.join(MDL_DIR, "adult_xgboost.pkl")
    ):
        adult_m = AdultMitigation().load()
        adult_tradeoff = adult_m.run()
    else:
        print(f"[ADULT MIT] {IN_ADULT} or adult_xgboost.pkl missing -- "
              f"skipping Adult section.")

    try:
        plot_mitigation_tradeoff(so_tradeoffs, fcc_tradeoff, adult_tradeoff)
    except Exception as e:
        print(f"  Warning: tradeoff plot failed -- {e}")

    try:
        plot_eu_compliance_table(so_tradeoffs, fcc_tradeoff, adult_tradeoff)
    except Exception as e:
